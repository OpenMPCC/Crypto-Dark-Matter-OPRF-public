...
impl<ROT: ROTSender<Msg = Block> + Malicious, FE: FF> Sender<ROT, FE> {
    ...
    pub fn send<C: AbstractChannel>(
        ...
        let mut w = FE::ZERO;
        let mut tau_list = Vec::with_capacity(self.nbits * self.pows.get().len());
        for (i, pow) in self.pows.get().iter().enumerate() {
            ...
            for (j, two) in self.twos.iter().enumerate() {
                ...
                let tau = w0 - w1 - *input;
                tau_list.push(tau);
                //channel.write_serializable(&(tau))?;
            }
            w += sum * *pow;
        }
        channel.write_serializable_seq(&tau_list)?;
        self.counter += 1;
        Ok(w)
    }
}
...
impl<ROT: ROTReceiver<Msg = Block> + Malicious, FE: FF> Receiver<ROT, FE> {
    ...
    pub fn init_with_ot<C: AbstractChannel, RNG: CryptoRng + Rng>(
        ...
        let delta = FE::random(&mut rng);
        Self::init_with_picked_delta_and_ot(channel, pows, rng, delta, ot)
    }
    ...
    pub fn receive<C: AbstractChannel>(&mut self, channel: &mut C) -> Result<FE, Error> {
        let pt = Block::from(self.counter as u128);
        let mut res = FE::ZERO;
        let mut tau_list = channel.read_serializable_seq::<FE::PrimeField>(self.nbits * self.pows.get().len())?;
        for (j, pow) in self.pows.get().iter().enumerate() {
            ...
            for (k, two) in self.twos.iter().enumerate() {
                ...
                let mut tau = tau_list.remove(0); 
                // channel.read_serializable::<FE::PrimeField>()?;
                let choice = Choice::from(self.choices[j * self.nbits + k] as u8);
                tau += w;
