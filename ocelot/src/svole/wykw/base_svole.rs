...
use crate::{errors::Error, ot::{endemic_ot, kos}};
use generic_array::typenum::Unsigned;
use rand::{CryptoRng, Rng, SeedableRng};
use scuttlebutt::{
    field::{Degree, FiniteField as FF}, ring::FiniteRing, AbstractChannel, AesRng
};
...
impl<FE: FF> Sender<FE> {
    ...
    /// Initalize the base vole sender with OT extension
    pub fn init_with_ot<C: AbstractChannel, RNG: CryptoRng + Rng>(
        channel: &mut C,
        pows: Powers<FE>,
        rng: &mut RNG,
        ot: kos::Sender<endemic_ot::Receiver>,
    ) -> Result<Self, Error> {
        let copee = CopeeSender::<FE>::init_with_ot(channel, pows.clone(), rng, ot)?;
        Ok(Self { copee, pows })
    }

    /// Extract the underlying COPEe sender
    pub fn extract_copee(&self) -> CopeeSender<FE> {
        self.copee.clone()
    }
...
impl<FE: FF> Receiver<FE> {
    ...
    /// Initalize the base vole receiver with OT extension
    pub fn init_with_ot<C: AbstractChannel, RNG: CryptoRng + Rng>(
        channel: &mut C,
        pows: Powers<FE>,
        rng: &mut RNG,
        ot: kos::Receiver<endemic_ot::Sender>,
    ) -> Result<Self, Error> {
        let cp = CopeeReceiver::<FE>::init_with_ot(channel, pows.clone(), rng, ot)?;
        Ok(Self { copee: cp, pows })
    }

    /// Extract the underlying COPEe receiver
    pub fn extract_copee(&self) -> CopeeReceiver<FE> {
        self.copee.clone()
    }
    ...
    /// Initalize the base vole receiver with a supplied `delta`
    pub fn init_with_picked_delta_and_ot<C: AbstractChannel, RNG: CryptoRng + Rng>(
        channel: &mut C,
        pows: Powers<FE>,
        rng: &mut RNG,
        delta: FE,
        ...
        let cp = CopeeReceiver::<FE>::init_with_picked_delta_and_ot(channel, pows.clone(), rng, delta, ot)?;
        Ok(Self { copee: cp, pows })
    }
