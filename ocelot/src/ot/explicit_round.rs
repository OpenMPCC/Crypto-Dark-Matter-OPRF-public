...
use scuttlebutt::{AbstractChannel, AesHash, AesRng, Block};
...
impl KosSender {
    ...
    pub fn send(
        ...
        for (j, (input, q)) in inputs.iter().zip(qs.chunks_exact(16)).enumerate() {
            ...
            let q = Block::from(q);
            let y0 = AesHash::fixed_key().tccr_hash(Block::from(j as u128), q) ^ input.0;
            let q = q ^ Block::from(self.alsz.s);
            let y1 = AesHash::fixed_key().tccr_hash(Block::from(j as u128), q) ^ input.1;
...
impl KosReceiverStage2 {
    ...
    pub fn stage2<'a>(
        ...
        let out = arena.alloc_slice_fill_with(choices.len(), |j| {
            ...
            y ^ AesHash::fixed_key().tccr_hash(Block::from(j as u128), Block::from(t))
        });
