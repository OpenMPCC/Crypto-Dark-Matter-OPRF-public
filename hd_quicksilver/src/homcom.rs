...
impl<FE: FiniteField> FComProver<FE> {
    ...
    pub fn init_from_previous_setup_sender<C: AbstractChannel, RNG: CryptoRng + Rng, FF: FiniteField>(
        channel: &mut C,
        rng: &mut RNG,
        lpn_setup: LpnParams,
        lpn_extend: LpnParams,
        ...
        let sender = Sender::<FE>::init_from_ot(channel, rng, lpn_setup, lpn_extend, sps)?;
        Ok(Self {
            svole_sender: sender,
            voles: Vec::new(),
            stats: Default::default(),
    ...
    pub fn init_from_previous_setup_receiver<C: AbstractChannel, RNG: CryptoRng + Rng, FF: FiniteField>(
        channel: &mut C,
        rng: &mut RNG,
        lpn_setup: LpnParams,
        lpn_extend: LpnParams,
        ...
        let sender = Sender::<FE>::init_from_ot_receiver(channel, rng, lpn_setup, lpn_extend, sps)?;
        Ok(Self {
            svole_sender: sender,
            voles: Vec::new(),
            stats: Default::default(),
    ...
    pub fn open<C: AbstractChannel>(
        ...
        let x_values = batch.iter().map(|x| x.value()).collect::<Vec<_>>();
        channel.write_serializable_seq::<FE::PrimeField>(&x_values)?;
...
impl<FE: FiniteField> FComVerifier<FE> {
    ...
    pub fn init_from_previous_setup_receiver<C: AbstractChannel, RNG: CryptoRng + Rng, FF: FiniteField>(
        ...
        let recv = Receiver::init_from_ot(channel, rng, lpn_setup, lpn_extend, sps)?;
        Ok(Self {
            delta: recv.delta(),
            svole_receiver: recv,
            voles: Vec::new(),
    ...
    pub fn init_from_previous_setup_sender<C: AbstractChannel, RNG: CryptoRng + Rng, FF: FiniteField>(
        ...
        let recv = Receiver::init_from_ot_sender(channel, rng, lpn_setup, lpn_extend, sps)?;
        Ok(Self {
            delta: recv.delta(),
            svole_receiver: recv,
            voles: Vec::new(),
    ...
    pub fn init_with_picked_delta<C: AbstractChannel, RNG: CryptoRng + Rng>(
        ...
        let recv = Receiver::init_with_picked_delta(channel, rng, lpn_setup, lpn_extend, delta)?;
        Ok(Self {
            delta: recv.delta(),
            svole_receiver: recv,
            voles: Vec::new(),
    ...
    pub fn init_with_picked_delta_and_ot<C: AbstractChannel, RNG: CryptoRng + Rng, FF: FiniteField>(
        ...
        let recv = Receiver::init_with_picked_delta_and_ot(channel, rng, lpn_setup, lpn_extend, delta, sps)?;
        Ok(Self {
            delta: recv.delta(),
            svole_receiver: recv,
            voles: Vec::new(),
    ...
    pub fn init_with_picked_delta_and_ot_receiver<C: AbstractChannel, RNG: CryptoRng + Rng, FF: FiniteField>(
        ...
        let recv = Receiver::init_with_picked_delta_and_ot_receiver(channel, rng, lpn_setup, lpn_extend, delta, sps)?;
        Ok(Self {
            delta: recv.delta(),
            svole_receiver: recv,
            voles: Vec::new(),
    ...
    pub fn open<C: AbstractChannel>(
        ...
        let mut hasher = blake3::Hasher::new();
        out.clear();
        let x_values = channel.read_serializable_seq::<FE::PrimeField>(keys.len())?;
        for i in 0..keys.len() {
            let x = x_values[i];
            out.push(x);
            hasher.update(&x.to_bytes());
...
// Extractors
impl<FE: FiniteField> FComProver<FE> {
    pub fn extract_svole_sender(&self) -> &Sender<FE> {
        &self.svole_sender
    }
}

impl<FE: FiniteField> FComVerifier<FE> {
    pub fn extract_svole_receiver(&self) -> &Receiver<FE> {
        &self.svole_receiver
    }
}
